...
def extract_traces(traces_file, labeled_traces_file, profiling_index=[n for n in range(0, 50000)],
    ...
    try:
        ...
        print(
            "Error: can't open HDF5 file '%s' for reading (it might be malformed) ... Exception '%s'" % traces_file % ex)
        sys.exit(-1)
    ...
    # 每一行是一条独立的trace, 提取列[min_target_point, max_target_point]
    ...
    for trace in attack_index:
        ...
        curr_trace += 1
    # Compute our labels
    labels_profiling = label(raw_plaintexts[profiling_index], raw_keys[profiling_index])
    labels_attack = label(raw_plaintexts[attack_index], raw_keys[attack_index])
